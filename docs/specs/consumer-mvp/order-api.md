...
| 필드 | 타입 | 필수 | 규칙 |
|---|---|---:|---|
| `clientRequestId` | `string` | Y | 주문 시도별 ULID/UUID, 같은 방문의 중복 요청 판정용 |
...
테이블 행을 먼저 잠그고 사용 여부를 확인한 뒤 방문 행과 주문번호 범위를 잠근다. 비활성화와 주문 생성이 동시에 발생하면 먼저 잠금을 획득해 커밋한 작업이 기준이 되며 주문이 일부만 저장되는 경우는 없다.
...
- 같은 `sysPlantCd + consumerSessionId + clientRequestId`와 같은 주문 내용은 성공 후 10분 동안 기존 `201` 결과를 재생한다.
- 같은 키에 다른 메뉴/옵션/수량을 보내면 `409`/`IDEMPOTENCY_PAYLOAD_MISMATCH`다.
- 같은 요청이 처리 중이면 잠시 결과를 기다리고, 제한 시간을 넘으면 `409`/`IDEMPOTENCY_IN_PROGRESS`다.
- 성공 후 10분이 지난 키는 삭제하지 않고 `409`/`IDEMPOTENCY_KEY_EXPIRED`로 막는다.

현재 저장소는 DB 스키마 권한이 없는 상태에서 QA를 진행하기 위한 단일 프로세스 메모리 구현이다. 서버 재시작이나 다중 서버 사이에서는 중복 요청을 막지 못하므로 운영 승격 전 DB/Redis 저장소와 고유 제약이 필요하다. 주문 생성은 별도 트랜잭션 서비스가 반환되어 commit된 뒤에만 성공 결과로 보관한다.
...
| `409` | 품절/가격 변경/허용되지 않은 상태. 테이블 비활성화 또는 멱등성 충돌은 안정적인 `error` 코드 제공 |
| `410` | 결제완료 또는 만료 방문 |
| `500` | 처리되지 않은 서버 오류 |

## 프런트 처리

1. 주문 시도마다 `clientRequestId`를 생성하고 네트워크 실패 재시도에서는 같은 값을 유지한다.
2. 제출 요청이 끝날 때까지 버튼을 비활성화한다.
3. 가격 변경 또는 품절 `409`면 장바구니를 유지하고 변경 내용을 안내한다.
4. `409`의 `error`가 `TABLE_INACTIVE`면 장바구니를 유지하고 주문 제출만 비활성화한다. 메뉴와 기존 주문 조회는 유지한다.
5. 성공하면 장바구니를 비우고 공유 주문목록 query를 갱신한다.
6. 별도 `추가 주문` 버튼 없이 메뉴 화면에서 다시 담는다.
7. `IDEMPOTENCY_IN_PROGRESS`/`IDEMPOTENCY_KEY_EXPIRED`/`IDEMPOTENCY_PAYLOAD_MISMATCH`에서는 장바구니와 키를 유지하고 자동 재주문 없이 주문내역 확인을 안내한다.
