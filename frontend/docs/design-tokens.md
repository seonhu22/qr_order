...
> CSS 커스텀 프로퍼티 기반 2단계 토큰 구조, 사용 원칙, 컬러·타이포그래피 참고표를 다룬다.
...
브라우저 기본 스크롤바는 두꺼워 보이므로 `global.css`에 얇은 스크롤바 스타일을 전역 적용한다.

- 스크롤바 두께: `--spacing-2`
- thumb 기본 색상: `--color-border-default`
- thumb 호버 색상: `--color-border-hover`
- thumb 모양: `--radius-pill` (둥근 형태)
- track 배경: `transparent`

모든 값은 semantic token을 사용한다. px 값 직접 사용 금지.
