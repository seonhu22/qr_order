...
모달에는 주문 단위 요청사항 입력(`requestNote`, 최대 200자)도 있다 — 메뉴별이 아니라 주문 전체에 하나다. "취소"를 눌러도 값은 유지되고, 주문이 실제로 완료되는 시점에만 비운다.
...
주문에 요청사항이 있으면(`order.requestNote`) 그 주문의 항목 목록 아래에 "요청사항: ..." 한 줄을 추가로 보여준다. 없으면 아예 표시하지 않는다.
